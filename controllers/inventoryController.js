...
exports.getProductById = async (req, res) => {
  try {
    // If you have a supplier reference, use .populate('supplier').
...
exports.addProduct = async (req, res) => {
  try {
    ...
    // Convert empty or "null" supplier to actual null (so Mongoose doesn't try to cast "null")
    if (supplier) {
      const existingSupplier = await Supplier.findById(supplier);
      if (!existingSupplier) {
        // Return error if there's no supplier with this ID
        return res
          .status(404)
